...
class AdminUserRoleController extends BaseController
{
    ...
    {
        $query = RbacAdminUserRole::find()->with('user')->with('role')->andWhere(['role_id'=>$roleId]);
    ...
    {
        if(count($ids) > 0){
            $c = RbacAdminUserRole::deleteAll(['in', 'id', $ids]);
    ...
    {
        if (($model = RbacAdminUserRole::findOne($id)) !== null) {
            return $model;
        } else {
