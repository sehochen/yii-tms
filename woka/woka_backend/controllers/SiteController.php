...
//             $this->layout = "lte_main";
    ...
    public function actionLogin()
    {
        ...
        if(false){
            echo json_encode(['errno'=>1, 'msg'=>'验证码不正确']);
        }
        else{
                ...
                if($login == true){
                    ...
                    echo json_encode(['errno'=>0, 'msg'=>$captcha, 'valid'=>$valid]);
                }
                else{
                    echo json_encode(['errno'=>2, 'msg'=>'用户名或密码不正确']);
                }
    ...
    {
        Yii::$app->admin->identity->clearUserSession();
        Yii::$app->admin->logout();
        return $this->goHome();
    }
    public function actionPsw()
    {
       $userRole = AdminUserRole::find()->with('role')->andWhere(['user_id'=>Yii::$app->admin->identity->id])->one();
    ...
    public function actionPswSave()
    {
        ...
        if(Yii::$app->admin->isGuest == false){
            $user = AdminUser::findByUsername(Yii::$app->admin->identity->username);
