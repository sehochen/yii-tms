...
use common\models\common\rbac\RbacAdminLog;
...
class BaseController extends Controller
{
    ...
    private function verifyPermission($action){
        ...
        if(Yii::$app->admin->isGuest){
            $allowUrl = ['site/error','index/index', 'index/login', 'index/captcha','index/reset-password'];
            if(in_array($route, $allowUrl) == false){
                echo '<script>
if(self!=top) top.location="/backend.php/index/login";
</script>';
                return false;
            ...
            $loginAllowUrl = ['site/error','index/index', 'index/logout', 'index/psw', 'index/psw-save','index/reset-password'];
            ...
            if(in_array($route, $loginAllowUrl) == false){
               ...
               if($route != 'system-log/index'){
                   $systemLog = new RbacAdminLog();
                    ...
                    $systemLog->create_user = Yii::$app->admin->identity->username;
