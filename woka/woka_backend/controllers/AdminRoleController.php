...
class AdminRoleController extends BaseController
{
    ...
    {
        if(count($ids) > 0){
            $c = RbacAdminRole::deleteAll(['in', 'id', $ids]);
    ...
    {
        if (($model = RbacAdminRole::findOne($id)) !== null) {
            return $model;
        } else {
    ...
    public function actionSaveRights(array $rids, $roleId){
        ...
        if(count($rids) > 0){
            ...
            $count = $adminRoleRightService->saveRights($rids, $roleId, Yii::$app->admin->identity->username);
            if($count > 0){
