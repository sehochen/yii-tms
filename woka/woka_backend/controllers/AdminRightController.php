...
class AdminRightController extends BaseController
{
    ...
    {
        $query = RbacAdminRight::find()->andWhere(['menu_id' => $id]);
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
              ...
              $model->create_user = Yii::$app->admin->identity->username;
              $model->create_date = date('Y-m-d H:i:s');
              $model->update_user = Yii::$app->admin->identity->username;
              $model->update_date = date('Y-m-d H:i:s');            
              if($model->validate() == true && $model->save()){
                  if(count($rightUrls) > 0){
                      $adminRightUrlService = new AdminRightUrlService();
                      $c = $adminRightUrlService->saveRightUrls($rightUrls, $model->id, Yii::$app->admin->identity->username);
                  }
    ...
    public function actionUpdate()
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            if($model->validate() == true && $model->save()){
                $adminRightUrlService = new AdminRightUrlService();
                $c = $adminRightUrlService->saveRightUrls($rightUrls, $id, Yii::$app->admin->identity->username);
                $msg = array('errno'=>0, 'msg'=>'保存成功');
                echo json_encode($msg);
            }
            else{
    ...
    {
        if(count($ids) > 0){
            $c = RbacAdminRight::deleteAll(['in', 'id', $ids]);
    ...
    {
        if (($model = RbacAdminRight::findOne($id)) !== null) {
            return $model;
        } else {
