...
class QrCode
{
     ...
     * @param string|bool $outfile the full file path to save as an image. If false will render an image. //保存图片路径，false则只显示图片
     * @param int $level the error correction level. Defaults to [[\dosamigos\qr\lib\Enum::QR_ECLEVEL_L]] (low) //图片纠错等级
     * @param int $size the size of the image. Defaults to 3.   尺寸大小的图像
     * @param int $margin the margin of the image. Defaults to 4. 利润图像的边缘
     * @param bool $saveAndPrint whether to also render the image even if we save it on a file. 是否呈现图片
     */
