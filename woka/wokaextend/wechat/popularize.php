...
class Popularize{
     ...
     * )
     */
    public static function createTicket($wxid,$type, $expireSeconds, $sceneId){
        $queryUrl = 'https://api.weixin.qq.com/cgi-bin/qrcode/create?access_token='.AccessToken::getAccessToken($wxid);
        $queryAction = 'POST';
        $template = array();
        if($type == 1){
     ...
     */
    public static function long2short($wxid,$longUrl){
        $queryUrl = 'https://api.weixin.qq.com/cgi-bin/shorturl?access_token='.AccessToken::getAccessToken($wxid);
        $queryAction = 'POST';
        $template = array();
