...
class KindEditor extends InputWidget {
    ...
    public function init() {
        ...
        $this->_options = [
            ...
            'uploadJson' => Url::to(['editor-upload', 'action' => 'uploadJson']),
            'width' => '100%',
            'height' => '400',
            'csrf' => Yii::$app->request->getCsrfToken(false),
        ...
        if ($this->hasModel()) {
            switch ($this->editorType) {
                ...
                case 'image-dialog':
                    ...
                    break;
                case 'image-dialog-simple':
                    return Html::activeInput('text', $this->model, $this->attribute, ['id' => $this->id]) . '<input type="button" id="imageBtn'.$this->id.'" value="选择图片" />';
            ...
            switch ($this->editorType) {
                ...
                case 'image-dialog':
                    ...
                    break;
                case 'image-dialog-simple':
                    return Html::input('text', $this->id, $this->value, ['id' => $this->id]) . '<input type="button" id="imageBtn'.$this->id.'" value="选择图片" />';
                    break;
                case 'file-dialog':
...
EOT;
            ...
            case 'image-dialog-simple':
				...
				K('#imageBtn{$this->id}').click(function() {
					editor.loadPlugin('image', function() {
						editor.plugin.imageDialog({
                            showRemote : false,
							imageUrl : K('#{$this->id}').val(),
