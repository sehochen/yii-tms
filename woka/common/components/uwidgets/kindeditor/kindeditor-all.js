...
_extend(KCmd, {
	...
	select : function(hasDummy) {
		...
		if (_IERANGE) {
			...
			rng = range.get(true);
//			sel.removeAllRanges(); //firefox 报错   edit time 20160710
//			sel.addRange(rng);		//firefox 报错 edit time 20160710
			if (doc !== document) {
...
KindEditor.plugin('image', function(K) {
	...
	self.plugin.imageDialog = function(options) {
		...
		var html = [
			...
			'<input type="button" class="ke-upload-button" value="' + lang.upload + '" />',
			'</div>',
			'<input type="hidden" name="_csrf" value="' + self.options.csrf + '" />',
			'</form>',
			'</div>',
			//local upload - end
			'</div>'
		...
		var dialog = self.createDialog({
			...
			yesBtn : {
				...
				click : function(e) {
					...
					// 多URL start 
					var remoteUrlsBox = K('.tab1 [name="remoteUrls[]"]');
					if( remoteUrlsBox != 'undefined' ){
						var width = widthBox.val(),
							height = heightBox.val(),
							title = titleBox.val(),
							align = '';
						remoteUrlsBox.each(function(index) {
							url = K("#remoteUrls" + index).val();
							console.log(index + " = " + url);
							if (url == 'http://' || K.invalidUrl(url)) {
								alert(self.lang('invalidUrl'));
								remoteUrlsBox[index].focus();
								return;
							}
							if (!/^\d*$/.test(width)) {
								alert(self.lang('invalidWidth'));
		...
		var uploadbutton = K.uploadbutton({
			...
			afterUpload : function(data) {
				dialog.hideLoading();
				if (data.error === 0) {
					var url = data.url; //单个URL
					
					var urlList = data.urlList; //多个URL
					console.log(urlList);
					if(urlList != undefined){
						K(".ke-dialog-row #remoteUrl", div).attr('type','hidden');
						urlList = "" + urlList+ "";
						var result = urlList.split(",");
//						console.log(result);
						for(var i=0;i < result.length;i++){
							...
							K('.ke-dialog-row #urlList').append('<p><input type="text" name="remoteUrls[]" id="remoteUrls'+ i +'" class="ke-input-text" name="url" value="'+url+'" style="width:200px;" /></p>');
		...
		refreshBtn.click(function(e) {
			var tempImg = K('<img src="' + urlBox.val() + '" /><br /', document).css({
				position : 'absolute',
				visibility : 'hidden',
				top : 0,
				left : '-1000px'
